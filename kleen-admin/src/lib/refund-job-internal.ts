...
export type RefundJobResult =
  | {
      ...
      cancelledAuthorization?: boolean;
      message?: string;
    }
  | { ok: false; error: string; status: number };

export async function refundJobInternal(params: RefundJobParams): Promise<RefundJobResult> {
  const stripeKey = process.env.STRIPE_SECRET_KEY;
  if (!stripeKey) {
    return { ok: false, error: "Stripe not configured", status: 503 };
  }
  const stripe = new Stripe(stripeKey);
  const { jobId, amountPence, reason, cancelAuthorizationOnly } = params;
  ...
  if (job.funds_released_at) {
    return {
      ok: false,
      error: "Funds were already released to the contractor. Use Stripe Dashboard for adjustments.",
      status: 400,
    };
  }
  ...
  if (!piId) {
    return { ok: false, error: "No Stripe payment on this job", status: 400 };
  }
  ...
  if (pi.status !== "succeeded") {
    return { ok: false, error: `Refund not available in state: ${pi.status}`, status: 400 };
  }

  const maxPence = pi.amount_received ?? pi.amount;
  const { data: pay } = await supabase
    .from("payments")
    .select("id, refund_amount_pence")
    .eq("job_id", jobId)
    .maybeSingle();
  const alreadyRefunded = pay?.refund_amount_pence ?? 0;
  const remaining = maxPence - alreadyRefunded;
  if (remaining <= 0) {
    return { ok: false, error: "Nothing left to refund on this charge", status: 400 };
  }
