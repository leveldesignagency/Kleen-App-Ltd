...
export async function releaseFundsForJob(jobId: string): Promise<ReleaseFundsResult> {
  ...
  if (job.funds_released_at) {
    return { ok: false, error: "Funds already released for this job", status: 400 };
  }
  if (!job.accepted_quote_request_id) {
    return { ok: false, error: "Job has no accepted quote", status: 400 };
  }
  ...
  if (!paymentCapturedAt) {
    return { ok: false, error: "Payment not yet authorized or captured for this job", status: 400 };
  }
  ...
  if (escrowRelease) {
    ...
    if (Number.isFinite(until) && Date.now() < until) {
      return {
        ok: false,
        error: `Dispute window active until ${new Date(escrowRelease).toISOString()}.`,
        status: 400,
      };
    }
  }
