  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-50 px-4 py-12">
      <div className="w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
        ...
        <h1 className="mt-4 text-xl font-bold text-slate-900">Account restricted</h1>
        <p className="mt-2 text-sm text-slate-600">
          Your Kleen account has been {ban?.ban_type === "temporary" ? "temporarily suspended" : "suspended"}.
          You cannot book jobs, open disputes, or use the dashboard until this is resolved.
        </p>
        {ban?.reason && (
          <div className="mt-4 rounded-xl bg-slate-50 p-4 text-sm text-slate-700">
            ...
            {ban.expires_at && (
              <p className="mt-2 text-xs text-slate-500">
                Review date: {new Date(ban.expires_at).toLocaleDateString("en-GB")}
              </p>
            )}
          </div>
        )}
